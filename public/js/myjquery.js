...
window.onload = function () {
  $(function () {
    var dateToMMDD = function dateToMMDD(date) {
      MM = date.getMonth() + 1;
      DD = date.getDate();
      return MM + '/' + DD;
    };

    var dateToHi = function dateToHi(date) {
      HH = ("0" + date.getHours()).slice(-2);
      ii = ("0" + date.getMinutes()).slice(-2);
      return HH + ':' + ii;
    };

    var dateToFullStr = function dateToFullStr(date) {
    ...
    var strToDate = function strToDate(timeStr, dateStr) {
    ...
    $('#record_grade').on('change', function () {
      ...
      $.ajax({
        ...
        //optionをいったん削除
        $('#record_kids a').remove(); //DBから受け取ったresponseからdataだけ取り出す

        data = data.data; //optionにセット
    ...
    $('.modal').on('show.bs.modal', function (e) {
      ...
      ;
      $('.btn-modal-submit').val(id); //idを渡して紐づく情報を受け取る
        ...
        if ($(this).attr('id') == 'edit-record') {
          ...
          $.each(guardians, function (key, value) {
            ...
            if (value.id == record.pu_guardian_id) {
              $('.pu_guardian select').append($('<option>').text(value.name).attr({
                'value': value.id,
                'selected': 'selected'
              }));
            } else {
          ...
          $('.do_time input').val(dateToHi(new Date(record.do_time))); //迎えに来た時間が記録されている場合のみ時間を表示
          ...
          $('.btn-modal-submit').val(); //削除ボタンが押された場合
        } else if ($(this).attr('id') == 'delete-record') {
          var record_date = new Date(record.created_at);
          $('.record-date').text(dateToMMDD(record_date)); //レコードから保護者の名前を取得

          var do_guardian_name = $.grep(guardians, function (key, value) {
            return key.id == record.do_guardian_id;
          })[0].name;
          var pu_guardian_name = ''; //迎えに来た人が記録されている場合は名前を取得
          ...
          $('.pu_guardian p').text(pu_guardian_name); //見やすいように日付をフォーマット

          $('.do_time p').text(dateToHi(new Date(record.do_time))); //迎えに来た時間が記録されている場合のみ時間を表示
          ...
          var lessons = kid.lessons; //曜日が一致する習い事のみ取り出す

          var lesson = $.grep(lessons, function (key, value) {
            return key.schedule == dayOfToday;
          })[0];
          $('.record-date').text(dateToMMDD(new Date())); //登園か降園か切り替える
          ...
          $('.do_guardian option').remove();
          $('.pu_guardian option').remove(); //習い事がある曜日は習い事に登録されている内容を規定値で表示させる
          ...
          if (lesson) {
            ...
            $('#pu_plan_hour').val(15);
            $('#pu_plan_minute').val(30); //optionをセット
    ...
    $('.btn-modal-submit').on('click', function (e) {
      ...
      if (route == 'record-store') {
        ...
        message = $('.record-kid').text() + 'さん おはようございます。'; //降園記録
        ...
        message = $('.record-kid').text() + 'さん　さようなら'; //記録編集
      ...
      $.ajax({
        ...
        //成功メッセージを表示
        $('.modal-body').prepend('<p class="alert alert-success" role="alert">' + message + '</p>'); //ボタンを閉じるボタンに

        $(this).text('閉じる'); //閉じるボタンを押したら

        $('.btn-modal-submit').on('click', function (e) {
          var modal = $(this).parents('.modal'); //閉じる

          modal.modal('hide'); //画面を更新

          location.reload();
        });
      }).fail(function (res) {
        //レスポンスからエラー内容を格納
        var res = res.responseJSON.errors; //エラーメッセージを宣言

        var alertList = ''; //登降園時間にエラーが出ている場合は

        if (res.do_time || res.pu_time) {
          alertList = '<li>すでに記録済みです</>'; //ボタンを閉じるボタンに
